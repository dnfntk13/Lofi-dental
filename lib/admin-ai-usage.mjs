...
export function estimateAdminAiCost(data) {
  ...
  const mini = /^gpt-5\.4-mini(?:-\d{4}-\d{2}-\d{2})?$/.test(data.model || '');
  const flagship = /^gpt-5\.4(?:-\d{4}-\d{2}-\d{2})?$/.test(data.model || '');
  if (!mini && !flagship) return null;
  if (data.service_tier && !['default', 'auto'].includes(data.service_tier)) return null;
  // https://developers.openai.com/api/docs/models/gpt-5.4
  const longContext = flagship && input > 272000;
  const inputRate = mini ? 0.75 : 2.5 * (longContext ? 2 : 1);
  const cachedRate = mini ? 0.075 : 0.25 * (longContext ? 2 : 1);
  const outputRate = mini ? 4.5 : 15 * (longContext ? 1.5 : 1);
  return ((input - cached) * inputRate + cached * cachedRate + output * outputRate) / 1e6;
}
