...
export function getAdminAiModel(env = process.env) {
  return String(env.ADMIN_AI_MODEL || '').trim() || 'gpt-5.4';
}

export function adminAiRequestOptions(model) {
  const flagship = /^gpt-5\.4(?:-\d{4}-\d{2}-\d{2})?$/.test(model);
  return {
    model,
    store: false,
    max_completion_tokens: flagship ? 24000 : 8000,
    ...(/^gpt-5(?:[.-]|$)/.test(model) ? { reasoning_effort: flagship ? 'high' : 'low' } : {}),
  };
}
